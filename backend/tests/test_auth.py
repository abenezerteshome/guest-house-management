...
@pytest.mark.asyncio
async def test_admin_and_reception_access(client: AsyncClient, users) -> None:
	...
	reception_response = await client.get(
		"/api/v1/users", headers={"Authorization": f"Bearer {reception_token}"}
	)
